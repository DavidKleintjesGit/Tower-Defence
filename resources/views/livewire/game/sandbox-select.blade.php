<div class="relative flex min-h-screen flex-col items-center px-4 py-10">
    ...
    <div class="relative z-10 flex w-full max-w-3xl flex-col items-center gap-8">
        ...
        <div class="text-center">
            <p class="text-sm uppercase tracking-[0.3em] text-emerald-400">Free experimentation</p>
            <h1 class="mt-2 text-3xl font-black uppercase tracking-widest text-emerald-300 drop-shadow-[0_0_18px_rgba(16,185,129,0.6)]">
                Sandbox
            </h1>
            <p class="mt-2 text-sm text-slate-400">Choose a map. No gold, no lives, no waves &mdash; freely place monsters and weapons.</p>
        </div>

        @if ($maps->isEmpty())
            <p class="text-sm text-slate-400">No maps are available yet.</p>
        @else
            <div class="grid w-full grid-cols-1 gap-4 sm:grid-cols-2">
                @foreach ($maps as $map)
                    ...
                    >
                        ...
                        <span class="text-xs text-slate-400">{{ $map->width }} &times; {{ $map->height }} tiles &middot; {{ $map->status }}</span>
                    </a>
                @endforeach
            </div>
        @endif
